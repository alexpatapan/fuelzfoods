...
def createPOrds(POrds, planType, columnName) :
    if df.at[i,'Item Name']=='planType' and df.at[i,'MPD']==1:
        for s in range(0, 5):
            POrds.at[s,'columnName'] += 1

    if df.at[i,'Item Name']=='planType' and df.at[i,'MPD']==2:
        for s in range(0, 10):
            POrds.at[s,'columnName'] += 1
...
for i in range(len(df['Order Number'])):
    createPOrds(POrds, "Subscription Plan", "Classic")
    createPOrds(POrds, "Asian Plan", "Asian")
